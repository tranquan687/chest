...
# Set up data loaders
train_data = Covid('/content/drive/MyDrive/Infection Segmentation Data/Infection Segmentation Data')
val_data = Covid('/content/drive/MyDrive/Infection Segmentation Data/Infection Segmentation Data',mode='val' )
...
#             f1_score(y_true, y_pred, average='macro')
    ...
    if iou_infected_meter.avg > best_acc: # best base on ind
        ...
        torch.save(model.state_dict(), "/content/drive/MyDrive/weight/sample_best.ckpt") # only save best to prevent output memory exceed error
#         torch.save(model,'best.pth')
        ...
        if stale > patience:
            ...
            break
    if epoch % save_every == 10 or epoch == num_epochs - 1:
        print(f"save model at epoch {epoch+1}, saving model")
        torch.save(model.state_dict(), f"/content/drive/MyDrive/weight/epoch_{epoch}.ckpt")